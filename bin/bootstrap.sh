...
if [ -f composer.lock ]; then
  ...
  COMPOSER_PROCESS_TIMEOUT=0 $PHP $PHPCOPTS composer.phar $DEVMODE update -o -a -vvv --ignore-platform-reqs --prefer-source --no-interaction
  echo "COMPOSER_PROCESS_TIMEOUT=0 $PHP $PHPCOPTS composer.phar $DEVMODE update -o -a --ignore-platform-reqs --prefer-source --no-interaction"
  ########### $PHP $PHPCOPTS composer.phar $DEVMODE update -o -a;
else
  ## $PHP $PHPCOPTS composer.phar config --global discard-changes true
  ## echo "$PHP $PHPCOPTS composer.phar config --global discard-changes true"
  COMPOSER_PROCESS_TIMEOUT=0 $PHP $PHPCOPTS composer.phar $DEVMODE install -o -a -vvv --ignore-platform-reqs --prefer-source --no-interaction
  echo "COMPOSER_PROCESS_TIMEOUT=0 $PHP $PHPCOPTS composer.phar $DEVMODE install -o -a --ignore-platform-reqs --prefer-source --no-interaction"
fi
...
################ FOR SYMFONY
if [ -f "$SCRIPT_PATH/../app/console" ]; then
  ($CD $SCRIPT_PATH/../ && $PHP $PHPCOPTS app/console cache:clear && $PHP $PHPCOPTS composer.phar dumpautoload)
fi
