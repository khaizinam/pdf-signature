...
command -v ffmpeg >/dev/null || {
    ...
    echo -e "${ROUGE}--> CENTOS: sudo sudo yum install ffmpeg ffmpeg-devel ${NORMAL}"
    # exit
}
...
asking_quality() {
    ...
    if [ -z "$COMPRESS_QUALITY" ]; then
        echo -e "Enter the quality percent, eg) 75, 85:"
        read -r quality
        while :; do
            if [[ -z $quality ]]; then
                echo -e "${ROUGE}--> Please enter your quality, such as:${ROUGE} 65${NORMAL}"
                read -r quality
            else
                # [[ -e $quality ]]
...
compress_webp() {
    ...
    if [ $FILE_SIZE -gt 0 ] && [ $FILE_SIZE -gt 250 ]; then
        # convert to webp
        # if [ "$MIME" == "jpg" ] || [ "$MIME" == "jpeg" ] || [ "$MIME" == "png" ] || [ "$MIME" == "gif" ]; then
        #     echo "${FILE_DIR}/${FILE_NAMEB}.webp"
        #     $CWEBP -progress -v -q ${COMPRESS_QUALITY} -mt "${1}" -o "${FILE_DIR}/${FILE_NAMEB}.webp" || true
        # fi
...
while :; do
    ...
    1)
        ...
        find ${SCREENSHOTS} \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" \) -print0 |
    ...
    2)
        ...
        # Find all (recursively) types of imagery JPG
        find ${SCREENSHOTS} \( -iname "*.jpg" -o -iname "*.jpeg" \) -print0 |
            while read -d $'\0' -r image; do
                # read w h < <(sips -g pixelWidth -g pixelHeight "$image" |
                #     awk '/Width:/{w=$2} /Height:/{h=$2} END{print w " " h}')
                # echo $image $w $h
    ...
    3)
        ...
        find ${SCREENSHOTS} \( -iname "*.png" \) -print0 |
            while read -d $'\0' -r image; do
                # read w h < <(sips -g pixelWidth -g pixelHeight "$image" |
                #     awk '/Width:/{w=$2} /Height:/{h=$2} END{print w " " h}')
                # echo $image $w $h
    ...
    4)
        ...
        find ${SCREENSHOTS} \( -iname "*.webp" \) -print0 |
            while read -d $'\0' -r image; do
                # read w h < <(sips -g pixelWidth -g pixelHeight "$image" |
                #     awk '/Width:/{w=$2} /Height:/{h=$2} END{print w " " h}')
                # echo $image $w $h
