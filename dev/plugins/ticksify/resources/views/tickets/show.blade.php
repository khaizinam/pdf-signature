    ...
    <div class="row">
        <div class="col-md-9">
            @if($ticket->is_resolved)
                <x-core::alert
                    type="success"
                    :title="trans('plugins/ticksify::ticksify.resolved_message')"
                />
            @endif
            ...
            <x-core::card class="mb-3">
                <x-core::card.header>
                    <x-core::card.title>
                        {{ trans('plugins/ticksify::ticksify.reply') }}
                    </x-core::card.title>
                </x-core::card.header>
                ...
                <x-core::card.body>
                    @if($ticket->is_locked)
                        <x-core::alert
                            type="warning"
                            :title="trans('plugins/ticksify::ticksify.locked_message')"
                        />
                    @endif
            ...
            @if($messages->isNotEmpty())
                <x-core::card class="mb-3">
                    <x-core::card.header>
                        <x-core::card.title>
                            {{ trans('plugins/ticksify::ticksify.replies') }}
                        </x-core::card.title>
                    </x-core::card.header>
                                                        ...
                                                        <div class="row">
                                                            <div class="col chat-bubble-author">
                                                                ...
                                                                @if($message->is_staff)
                                                                    <x-core::badge
                                                                        color="primary"
                                                                        :label="trans('plugins/ticksify::ticksify.staff')"
                                                                    />
                                                                @endif
                                                            </div>
        ...
        <div class="col-md-3">
            <x-core::card>
                ...
                <x-core::card.body>
                    <dl class="row">
                        <dt class="col-5">{{ trans('plugins/ticksify::ticksify.ticket_id') }}:</dt>
                        <dd class="col-7">#{{ $ticket->getKey() }}</dd>
                        <dt class="col-5">{{ trans('plugins/ticksify::ticksify.priority') }}:</dt>
                        <dd class="col-7">{!! $ticket->priority->toHtml() !!}</dd>
                        <dt class="col-5">{{ trans('plugins/ticksify::ticksify.category') }}:</dt>
                        <dd class="col-7">{{ $ticket->category->name }}</dd>
                        <dt class="col-5">{{ trans('plugins/ticksify::ticksify.created_at') }}:</dt>
                        <dd class="col-7">
                            <time title="{{ $ticket->created_at->translatedFormat('d M Y H:i') }}">{{ $ticket->created_at->diffForHumans() }}</time>
                        </dd>
                        <dt class="col-5">{{ trans('plugins/ticksify::ticksify.updated_at') }}:</dt>
                        <dd class="col-7">
                ...
                <x-core::card.footer>
                    ...
                    >
                        {{ trans('plugins/ticksify::ticksify.update_ticket') }}
                    </x-core::button>
                </x-core::card.footer>
            </x-core::card>
        </div>
...
@push('footer')
    <x-core::modal
        ...
        size="lg"
        :title="trans('plugins/ticksify::ticksify.update_ticket')"
    >
