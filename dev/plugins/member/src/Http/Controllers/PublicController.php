...
use Dev\Member\Forms\Fronts\ChangePasswordForm;
use Dev\Member\Forms\Fronts\ProfileForm;
use Dev\Member\Http\Requests\AvatarRequest;
use Dev\Member\Http\Requests\SettingRequest;
use Dev\Member\Http\Requests\UpdatePasswordRequest;
use Dev\Member\Http\Resources\ActivityLogResource;
...
class PublicController extends BaseController
{
    public function getAuthor(string $slug)
    {
        ...
        if ($author->avatar) {
            $meta->setImage(AppMedia::getImageUrl($author->avatar));
        }
    ...
    public function postAvatar(AvatarRequest $request)
    {
        try {
            ...
            $result = AppMedia::uploadFromBlob($request->file('avatar_file'), folderSlug: $account->upload_folder);
            ...
            return $this
                ...
                ->setData(['url' => AppMedia::url($file->url)]);
    ...
    public function postUpload(Request $request)
    {
        ...
        if (! AppMedia::isChunkUploadEnabled()) {
            $validator = Validator::make($request->all(), [
                'file.0' => AppMedia::imageValidationRule(),
            ]);
            ...
            $result = AppMedia::handleUpload(Arr::first($request->file('file')), 0, $account->upload_folder);
        ...
        try {
            ...
            if ($save->isFinished()) {
                $result = AppMedia::handleUpload($save->getFile(), 0, $account->upload_folder);
    ...
    public function postUploadFromEditor(Request $request)
    {
        ...
        return AppMedia::uploadFromEditor($request, 0, $account->upload_folder);
    }
}
