...
class Apps {
    ...
    constructor() {
        ...
        Apps.initNavbarMinimal()
        Apps.initResources()
        Apps.initGlobalResources()
        Apps.handleCounterUp()
        Apps.initMediaIntegrate()

        if (
            AppVars &&
            AppVars.authorized === '0' &&
            typeof AppsVariables.authorize_url !== 'undefined'
        ) {
    ...
    static blockUI(options) {
        ...
        Apps.showLoading(options.target)
    }

    static unblockUI(target) {
        Apps.hideLoading(target)
    }
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (Apps.noticesTimeout[key]) {
            clearTimeout(Apps.noticesTimeout[key])
        }

        Apps.noticesTimeout[key] = setTimeout(() => {
            if (!messageHeader) {
                switch (messageType) {
                    case 'error':
                        messageHeader = AppVars.languages.notices_msg.error
                        break
                    case 'success':
                        messageHeader = AppVars.languages.notices_msg.success
                        break
                }
            }
    ...
    static handleError(data) {
        if (typeof data.errors !== 'undefined' && !_.isArray(data.errors)) {
            Apps.handleValidationError(data.errors)
        } else {
            if (typeof data.responseJSON !== 'undefined') {
                if (typeof data.responseJSON.errors !== 'undefined') {
                    if (data.status === 422) {
                        Apps.handleValidationError(data.responseJSON.errors)
                    }
                } else if (typeof data.responseJSON.message !== 'undefined') {
                    Apps.showError(data.responseJSON.message)
                } else {
                ...
                Apps.showError(data.statusText)
            }
        }
    }
    ...
    static async copyToClipboard(textToCopy, parentTarget) {
        if (navigator.clipboard && window.isSecureContext) {
            ...
            Apps.unsecuredCopyToClipboard(textToCopy, parentTarget)
        }
    }
    ...
    static initResources() {
        ...
        Apps.initDatePicker('.datepicker')

        if (jQuery().textareaAutoSize) {
            $('textarea.textarea-auto-height').textareaAutoSize()
        }

        Apps.initCodeEditorComponent()
        Apps.initColorPicker()
        Apps.initLightbox()
        Apps.initTreeCategoriesSelect()
        Apps.initCoreIcon()
    ...
    static initGlobalResources() {
        ...
        Apps.initFieldCollapse()
        Apps.initTreeCheckboxes()
        Apps.initClipboard()
        Apps.initDropdownCheckboxes()
    }
    ...
    handleTurnOffDebugMode() {
        ...
        $submit.on('click', function (event) {
            ...
            Apps.showButtonLoading($submit[0])
            ...
            $httpClient
                ...
                .finally(() => {
                    Apps.hideButtonLoading($submit[0])
                })
        })
    }
    ...
    static initMediaIntegrate() {
        if (jQuery().appMedia) {
            Apps.gallerySelectImageTemplate = `
            <div class='custom-image-box image-box'>
                <input type='hidden' name='__name__' value='' class='image-data'>
                    ...
                    <div class='preview-image-inner'>
                        <img src='${APP_MEDIA_CONFIG.default_image}' alt='${APP_MEDIA_CONFIG.translations.preview_image}' class='preview-image'>
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Apps.gallerySelectImageTemplate
            ...
            new AppMediaStandAlone('[data-bb-toggle="gallery-add"]', {
                filter: 'image',
                view_in: 'all_media',
                onSelectFiles: (files, $el) => {
            ...
            new AppMediaStandAlone('[data-bb-toggle="image-picker-edit"]', {
                filter: 'image',
                view_in: 'all_media',
                onSelectFiles: (files, $el) => {
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown((event) => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Apps.onDragTool)
            $(window).on('mouseup', Apps.onReleaseTool)
        })
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Apps.onDragTool)
        $(window).off('mouseup', Apps.onReleaseTool)
    }

    processAuthorize() {
        $httpClient
            .makeWithoutErrorHandler()
            .post(AppVars.authorize_url)
            .catch(() => {})
    }
    ...
    countMenuItemNotifications() {
        ...
        if ($menuItems.length) {
            $httpClient
                .make()
                .get($menuItems.data('url') || AppVars.menu_item_count_url)
                .then(({ data }) => {
                    data.data.map((x) => {
                        if (x.value > 0) {
    ...
    static initFieldCollapse() {
        $(document).on('click, change', '[data-bb-toggle="collapse"]', function (e) {
            ...
            switch (e.currentTarget.type) {
                ...
                default:
                    console.warn(`[Dev] Unknown type ${e.currentTarget.type} of collapse`)
    ...
    static initCodeEditorComponent() {
        $(document)
            ...
            .each(function () {
                ...
                Apps.initCodeEditor(this, this.dataset.mode || 'htmlmixed')
            })
    }
    ...
    static initLightbox() {
        ...
        a.forEach((element) => {
            ...
            if (!instance[instanceName]) {
                instance[instanceName] = Apps.lightbox()
            }
    ...
    static initClipboard() {
        $(document).on('click', '[data-bb-toggle="clipboard"]', async (e) => {
            ...
            await Apps.copyToClipboard(text, clipboardParentTarget)

            if (copiedMessage) {
                Apps.showSuccess(copiedMessage)
            }
        ...
        $element.editable({
            ...
            success: function (response) {
                if (response.error && response.message) {
                    Apps.showError(response.message)
                }
            },
            error: function (response) {
                Apps.handleError(response)
            },
        })
    }
