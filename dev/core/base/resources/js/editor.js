...
class EditorManagement {
    ...
    async ckFinderInitial(editor, element) {
        ...
        if (ckFileRepository && APP_MEDIA_URL.media_upload_from_editor) {
            ckFileRepository.createUploadAdapter = (loader) => {
                return new CKEditorUploadAdapter(loader, APP_MEDIA_URL.media_upload_from_editor, editor.t)
            }
        }
        ...
        if (ckfinder && btnGalleries.length) {
            ...
            ckfinder.execute = () => Apps.showError('Not available.')
        }
    }
    ...
    uploadImageFromEditor(blobInfo, callback) {
        ...
        $httpClient
            .make()
            .postForm(APP_MEDIA_URL.media_upload_from_editor, formData)
            .then(({ data }) => {
                if (data.uploaded) {
                    callback(data.url)
                }
    ...
    async initTinyMce(element) {
        let options = {
            ...
            images_upload_url: APP_MEDIA_URL.media_upload_from_editor,
