...
{
    public const REPOSITORY = 'vswb/translations';
    ...
    public function handle(string $locale): void
    {
        ...
        File::ensureDirectoryExists(lang_path("vendor/ui/{$parentTheme}"));

        if (File::exists("{$path}/{$locale}.json") && ! File::exists(lang_path("vendor/ui/{$parentTheme}/{$locale}.json"))) {
            File::copy("{$path}/{$locale}.json", lang_path("vendor/ui/{$parentTheme}/{$locale}.json"));
        }
