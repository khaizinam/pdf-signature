...
class EmailHandler
{
    ...
    protected string $platform_path = 'dev';
	...
	public function __construct(public string $platform_type = 'dev')
    {
    ...
    protected function getSiteLogo(): string
    {
        ...
        return $siteLogo
            ? AppMedia::getImageUrl($siteLogo)
            : (
                ($adminLogo = setting('admin_logo'))
                ? AppMedia::getImageUrl($adminLogo)
    ...
    protected function getSiteSocialLinks(): array
    {
        return ($socialLinks = setting('email_template_social_links'))
            ? collect(json_decode($socialLinks, true))
                ...
                ->map(function ($link) {
                    $link['image'] = AppMedia::getImageUrl($link['image']);
    ...
    public function addTemplateSettings(string $module, ?array $data, string $type = 'plugins'): self
    {
        ...
        foreach ($data['templates'] as $key => &$template) {
            ...
            $this->templates[$type][$module]['templates'][$key]['path'] = platform_path(
                $type . '/' . $module . '/resources/email-templates/' . $key . '.tpl', $this->getPlatformPath()
            );
        }
    ...
    public function getTemplateContent(string $template, string $type = 'plugins'): ?string
    {
        ...
        return get_setting_email_template_content($type, $this->module, $template, $this->getPlatformPath());
    }
    ...
    {
        $content = $this->prepareData(get_setting_email_template_content($this->type, $this->module, $this->template, $this->getPlatformPath()));
