...
class MediaFile extends BaseModel
{
    ...
    {
        static::forceDeleted(fn (MediaFile $file) => AppMedia::deleteFile($file));

        static::addGlobalScope('ownMedia', function (Builder $query) {
            if (AppMedia::canOnlyViewOwnMedia()) {
    ...
    protected function type(): Attribute
    {
        return Attribute::make(
            get: function ($value, $attributes) {
                ...
                foreach (AppMedia::getConfig('mime_types', []) as $key => $value) {
            ...
            switch ($this->type) {
                ...
                case 'document':
                    ...
                    if (
                        ...
                        $preview = Str::replace('{url}', urlencode(AppMedia::url($this->url)), $url);
                    }
    ...
    {
        return (! $this->visibility || $this->visibility === 'public') && AppMedia::canGenerateThumbnails($this->mime_type);
    }
    ...
    public static function createSlug(string $name, string $extension, ?string $folderPath): string
    {
        if (setting('media_use_original_name_for_file_path')) {
            ...
            $slug = Str::slug($name, '-', ! AppMedia::turnOffAutomaticUrlTranslationIntoLatin() ? 'en' : false);
        }

        $index = 1;
        $baseSlug = $slug;
        while (File::exists(AppMedia::getRealPath(rtrim($folderPath, '/') . '/' . $slug . '.' . $extension))) {
            $slug = $baseSlug . '-' . $index++;
        }
