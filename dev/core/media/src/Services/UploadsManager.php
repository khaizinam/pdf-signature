...
class UploadsManager
{
    public function fileDetails(string $path): array
    {
        return [
            ...
            'url' => $path,
            'mime_type' => $this->fileMimeType(AppMedia::getRealPath($path)),
    ...
    {
        return AppMedia::getMimeType($path);
    }
    ...
    public function saveFile(
        ...
        $storage = Storage::disk(AppMedia::getConfig('disk'));

        if ($visibility === 'private' && ! AppMedia::isUsingCloud()) {
            $storage = Storage::disk('local');
        }

        if (! AppMedia::isChunkUploadEnabled() || ! $file) {
            return $storage->put($this->cleanFolder($path), $content);
        }

        $currentChunksPath = AppMedia::getConfig('chunk.storage.chunks') . '/' . $file->getFilename();
        $disk = Storage::disk(AppMedia::getConfig('chunk.storage.disk'));
