...
use Dev\Media\Http\Requests\MediaListRequest;
...
class MediaController extends BaseController
{
    ...
    {
        $response = AppMedia::responseError(trans('core/media::media.invalid_action'));
        ...
        switch ($type) {
            case 'trash':
                ...
                if ($error) {
                    $response = AppMedia::responseError(trans('core/media::media.trash_error'));

                    break;
                }

                $response = AppMedia::responseSuccess([], trans('core/media::media.trash_success'));
            ...
            case 'restore':
                ...
                if ($error) {
                    $response = AppMedia::responseError(trans('core/media::media.restore_error'));

                    break;
                }

                $response = AppMedia::responseSuccess([], trans('core/media::media.restore_success'));
            ...
            case 'make_copy':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.copy_success'));
            ...
            case 'delete':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.delete_success'));
            ...
            case 'favorite':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.favorite_success'));
            ...
            case 'remove_favorite':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.remove_favorite_success'));
            ...
            case 'crop':
                ...
                if (! $file->canGenerateThumbnails()) {
                    $response = AppMedia::responseError(trans('core/media::media.failed_to_crop_image'));
                ...
                try {
                    $thumbnailService
                        ->setImage(AppMedia::getRealPath($fileUrl))
                    ...
                    if (! AppMedia::isUsingCloud()) {
                        $message = trans('core/media::media.unable_to_write', ['folder' => AppMedia::getUploadPath()]);
                    }

                    return AppMedia::responseError($message);
                ...
                AppMedia::generateThumbnails($file);

                $response = AppMedia::responseSuccess([], trans('core/media::media.crop_success'));
            ...
            case 'rename':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.rename_success'));
            ...
            case 'alt_text':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.update_alt_text_success'));
            ...
            case 'empty_trash':
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.empty_trash_success'));

                break;

            case 'properties':
                Validator::validate($request->input(), [
                    'color' => ['required', 'string', Rule::in(AppMedia::getFolderColors())],
                    'selected' => ['required', 'array'],
                ...
                $response = AppMedia::responseSuccess([], trans('core/media::media.update_properties_success'));
    ...
    protected function copyFile(MediaFile $file, int|string|null $newFolderId = null)
    {
        ...
        if ($newFolderId == null) {
            ...
            $filePath = AppMedia::getRealPath($file->url);
            ...
            $file->url = str_replace(
                AppMedia::getRealPath(File::dirname($file->url)),
                AppMedia::getRealPath($this->folderRepository->getFullPath($newFolderId)),
                $file->url
            );
        ...
        if (count($items) == 1 && ! $items[0]['is_folder']) {
            ...
            if (! empty($file) && $file->type != 'video') {
                ...
                if (! AppMedia::isUsingCloud()) {
                    if (! File::exists($filePath)) {
                        return AppMedia::responseError(trans('core/media::media.file_not_exists'));
                    }
            ...
            foreach ($items as $item) {
                ...
                if (! $item['is_folder']) {
                    ...
                    if (! empty($folder)) {
                        if (! AppMedia::isUsingCloud()) {
                            $folderPath = AppMedia::getRealPath($this->folderRepository->getFullPath($folder->id));
                            ...
                            foreach ($allFiles as $file) {
                                $zip->addString(
                                    File::basename($file),
                                    Http::withoutVerifying()->get(AppMedia::getRealPath($file))->body()
                                );
                            }
                        }
                    }
            ...
            return AppMedia::responseError(trans('core/media::media.download_file_error'));
        }

        return AppMedia::responseError(trans('core/media::media.can_not_download_file'));
    }
}
