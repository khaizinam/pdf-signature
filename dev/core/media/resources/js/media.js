...
class MediaManagement {
    ...
    setupLayout() {
         ...
         */
        const $currentFilter = $(
            `.js-app-media-change-filter[data-type="filter"][data-value="${Helpers.getRequestParams().filter}"]`
        )
        ...
        $currentFilter
            ...
            .addClass('active')
            .closest('.dropdown')
            .find('.js-app-media-filter-current')
            .html(`(${$currentFilter.html()})`)

        const $currentViewIn = $(
            `.js-app-media-change-filter[data-type="view_in"][data-value="${Helpers.getRequestParams().view_in}"]`
        )
        ...
        if (Helpers.isUseInModal()) {
            $('.app-media-footer').removeClass('d-none')
        }

        /**
         * Sort
         */
        $(`.js-app-media-change-filter[data-type="sort_by"][data-value="${Helpers.getRequestParams().sort_by}"]`)
            .closest('button.dropdown-item')
            .addClass('active')
    ...
    handleMediaList() {
        ...
        _self.$body
            ...
            .on('click', '.js-media-list-title', (event) => {
                ...
                if (shift_key) {
                    ...
                    if (firstItem) {
                        ...
                        $('.app-media-items li').each((index, el) => {
                    ...
                    $current.closest('.app-media-items').find('input[type=checkbox]').prop('checked', false)
                }
            ...
            .on('dblclick doubletap', '.js-up-one-level', (event) => {
                event.preventDefault()
                let count = $('.app-media-breadcrumb .breadcrumb li').length
                $(`.app-media-breadcrumb .breadcrumb li:nth-child(${count - 1}) a`).trigger('click')
            })
            ...
            .on('click contextmenu', '.app-media-items', (e) => {
                if (!Helpers.size(e.target.closest('.js-context-menu'))) {
                    $('.app-media-items input[type="checkbox"]').prop('checked', false)
    ...
    changeViewType() {
        let _self = this
        _self.$body
            .off('click', '.js-app-media-change-view-type button')
            .on('click', '.js-app-media-change-view-type button', (event) => {
                event.preventDefault()
                ...
                $current.closest('.js-app-media-change-view-type').find('button').removeClass('active')
                ...
                if (typeof APP_MEDIA_CONFIG.pagination != 'undefined') {
                    if (typeof APP_MEDIA_CONFIG.pagination.paged != 'undefined') {
                        APP_MEDIA_CONFIG.pagination.paged = 1
                    }
                }
        ...
        $(`.js-app-media-change-view-type .btn[data-type="${Helpers.getRequestParams().view_type}"]`).trigger('click')
    ...
    changeFilter() {
        let _self = this
        _self.$body.off('click', '.js-app-media-change-filter').on('click', '.js-app-media-change-filter', (event) => {
            event.preventDefault()
            ...
            if (!Helpers.isOnAjaxLoading()) {
                ...
                if (window.appMedia.options && data.type === 'view_in') {
                    window.appMedia.options.view_in = data.value
                }
                ...
                $current.closest('.dropdown').find('.js-app-media-filter-current').html(`(${$current.html()})`)
    ...
    handleActions() {
        ...
        _self.$body
            .off('click', '.app-media-actions .js-change-action[data-type="refresh"]')
            .on('click', '.app-media-actions .js-change-action[data-type="refresh"]', (event) => {
                event.preventDefault()

                Helpers.resetPagination()

                let ele_options =
                    typeof window.appMedia.$el !== 'undefined' ? window.appMedia.$el.data('app-media') : undefined
                if (
            ...
            .off('click', '.app-media-items li.no-items')
            .on('click', '.app-media-items li.no-items', (event) => {
                event.preventDefault()
                $('.app-media-header .app-media-top-header .app-media-actions .js-dropzone-upload').trigger('click')
            })
    ...
    handleModals() {
        ...
        _self.$body
            ...
            .on('click', '#modal-properties button[type="submit"]', (event) => {
                ...
                Apps.showButtonLoading($modal.find('button[type="submit"]'))
                ...
                ActionsService.processAction(
                    ...
                    () => {
                        $modal.modal('hide')
                        Apps.hideButtonLoading($modal.find('button[type="submit"]'))
        ...
        _self.$body
            ...
            .on('submit', '#modal_crop_image .form-crop', (event) => {
                ...
                Apps.showButtonLoading($form.find('button[type="submit"]'))
                ...
                ActionsService.processAction(
                    ...
                    (response) => {
                        ...
                        Apps.hideButtonLoading($form.find('button[type="submit"]'))
                    }
                )
            })
    ...
    checkFileTypeSelect(selectedFiles) {
        if (typeof window.appMedia.$el !== 'undefined') {
            let firstItem = Helpers.arrayFirst(selectedFiles)
            let ele_options = window.appMedia.$el.data('app-media')
            if (
                typeof ele_options !== 'undefined' &&
    ...
    bindIntegrateModalEvents() {
        let $mainModal = $('#app_media_modal')
        let _self = this
        $mainModal.off('click', '.js-insert-to-editor').on('click', '.js-insert-to-editor', (event) => {
            event.preventDefault()
            let selectedFiles = Helpers.getSelectedFiles()
            if (Helpers.size(selectedFiles) > 0) {
                window.appMedia.options.onSelectFiles(selectedFiles, window.appMedia.$el)
    ...
    scrollGetMore() {
        let _self = this
        $('.app-media-main .app-media-items').bind('DOMMouseScroll mousewheel', (e) => {
            if (e.originalEvent.detail > 0 || e.originalEvent.wheelDelta < 0) {
                ...
                if (loadMore) {
                    if (typeof APP_MEDIA_CONFIG.pagination != 'undefined' && APP_MEDIA_CONFIG.pagination.has_more) {
