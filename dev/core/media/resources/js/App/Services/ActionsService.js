...
export class ActionsService {
    static handleDropdown() {
        ...
        if (selected > 0) {
            $('.app-dropdown-actions > .dropdown-toggle').removeClass('disabled').prop('disabled', false)
        } else {
            $('.app-dropdown-actions > .dropdown-toggle').addClass('disabled').prop('disabled', true)
        }
    }
    ...
    static renderCropImage() {
        const html = $('#app_media_crop_image').html()
    ...
    static async handleCopyLink() {
        ...
        await Apps.copyToClipboard(links)
    ...
    static handleGlobalAction(type, callback) {
        ...
        switch (type) {
            ...
            case 'alt_text':
                ...
                break
            case 'crop':
                $('#modal_crop_image').modal('show').find('form.app-form').data('action', type)
                break
            case 'trash':
    ...
    static processAction(data, callback = null) {
        ...
        $httpClient
            .make()
            .post(APP_MEDIA_URL.global_actions, data)
            .then(({ data }) => {
    ...
    static renderRenameItems() {
        let VIEW = $('#app_media_rename_item').html()
    ...
    static renderAltTextItems() {
        let VIEW = $('#app_media_alt_text_item').html()
    ...
    static renderActions() {
        ...
        let ACTION_TEMPLATE = $('#app_action_item').html()
        let initializedItem = 0
        let $dropdownActions = $('.app-dropdown-actions .dropdown-menu')
    ...
    static handleDownload(files) {
        ...
        $httpClient
            ...
            .post(APP_MEDIA_URL.download, { selected: files })
            .then((response) => {
