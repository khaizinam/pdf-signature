...
export class UploadService {
    constructor() {
        ...
        this.uploadUrl = APP_MEDIA_URL.upload_file

        this.uploadProgressBox = $('.app-upload-progress')

        this.uploadProgressContainer = $('.app-upload-progress .app-upload-progress-table')

        this.uploadProgressTemplate = $('#app_media_upload_progress_item').html()
    ...
    init() {
        if (Helpers.hasPermission('files.create') && $('.app-media-items').length > 0) {
            this.setupDropZone()
        }
        this.handleEvents()
    }
    ...
    setupDropZone() {
        ...
        _self.dropZone = new Dropzone(document.querySelector('.app-media-items'), {
            ..._dropZoneConfig,
        ...
        _self.dropZone.on('queuecomplete', () => {
            ...
            if (_self.totalError === 0) {
                setTimeout(() => {
                    $('.app-upload-progress .close-pane').trigger('click')
                }, 5000)
            }
        })
    }
    ...
    handleEvents() {
         ...
         */
        _self.$body
            .off('click', '.app-upload-progress .close-pane')
            .on('click', '.app-upload-progress .close-pane', (event) => {
                event.preventDefault()
                $('.app-upload-progress').addClass('hide-the-pane')
                _self.totalError = 0
                setTimeout(() => {
                    $('.app-upload-progress tr').remove()
                    _self.totalQueued = 1
                }, 300)
            })
    }
    ...
    getDropZoneConfig() {
        return {
            url: this.uploadUrl,
            uploadMultiple: !APP_MEDIA_CONFIG.chunk.enabled,
            chunking: APP_MEDIA_CONFIG.chunk.enabled,
            ...
            chunkSize: APP_MEDIA_CONFIG.chunk.chunk_size, // chunk size 1,000,000 bytes (~1MB)
            retryChunks: true, // retry chunks on failure
            retryChunksLimit: 3, // retry maximum of 3 times (default is 3)
            timeout: 0, // MB,
            maxFilesize: APP_MEDIA_CONFIG.chunk.max_file_size, // MB
