...
export class MediaService {
    constructor() {
        ...
        this.breadcrumbTemplate = $('#app_media_breadcrumb_item').html()
    }

    getMedia(reload = false, is_popup = false, load_more_file = false) {
        if (typeof APP_MEDIA_CONFIG.pagination != 'undefined') {
            if (APP_MEDIA_CONFIG.pagination.in_process_get_media) {
                return
            }

            APP_MEDIA_CONFIG.pagination.in_process_get_media = true
        }
        ...
        if (typeof APP_MEDIA_CONFIG.pagination != 'undefined') {
            params = {
                ...params,
                paged: APP_MEDIA_CONFIG.pagination.paged,
                posts_per_page: APP_MEDIA_CONFIG.pagination.posts_per_page,
            }
        }
        ...
        $httpClient
            .make()
            .get(APP_MEDIA_URL.get_media, params)
            .then(({ data }) => {
                ...
                if (typeof APP_MEDIA_CONFIG.pagination != 'undefined') {
                    ...
                    if (typeof APP_MEDIA_CONFIG.pagination.in_process_get_media != 'undefined') {
                        APP_MEDIA_CONFIG.pagination.in_process_get_media = false
                    }

                    if (
                        typeof APP_MEDIA_CONFIG.pagination.posts_per_page != 'undefined' &&
                        data.data.files.length + data.data.folders.length < APP_MEDIA_CONFIG.pagination.posts_per_page &&
                        typeof APP_MEDIA_CONFIG.pagination.has_more != 'undefined'
                    ) {
                        APP_MEDIA_CONFIG.pagination.has_more = false
                    }
                }
            })
    ...
    renderBreadcrumbs(breadcrumbItems) {
        let _self = this
        let $breadcrumbContainer = $('.app-media-breadcrumb .breadcrumb')
        ...
        $('.app-media-container').attr('data-breadcrumb-count', Helpers.size(breadcrumbItems))
    }

    static refreshFilter() {
        const $appMediaContainer = $('.app-media-container')
        const viewIn = Helpers.getRequestParams().view_in
        const $actionsTarget = $('.app-media-actions .btn:not([data-type="refresh"]):not([data-bs-toggle="offcanvas"])')

        if (viewIn !== 'all_media' && !Helpers.getRequestParams().folder_id) {
            $actionsTarget.addClass('disabled')
            $appMediaContainer.attr('data-allow-upload', 'false')
        } else {
            $actionsTarget.removeClass('disabled')
            $appMediaContainer.attr('data-allow-upload', 'true')
        }

        $('.app-media-actions .btn.js-app-media-change-filter-group').removeClass('disabled')

        const $emptyTrashBtn = $('.app-media-actions .btn[data-action="empty_trash"]')
        ...
        $appMediaContainer.attr('data-view-in', viewIn)
    }
}
