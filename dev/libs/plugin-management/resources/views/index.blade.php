...
@push('header-action')
    @if (
        $isEnabledMarketplaceFeature = config('libs.plugin-management.general.enable_marketplace_feature')
        ...
        <x-core::button
            ...
            class="ms-auto"
        >
            {{ trans('libs/plugin-management::plugin.plugins_add_new') }}
        </x-core::button>
    @endif
        ...
        <div class="d-flex gap-2 justify-content-between">
            <div class="w-100 w-sm-25">
                <x-core::form.text-input
                    type="search"
                    name="search"
                    :placeholder="trans('libs/plugin-management::plugin.search')"
                    :group-flat="true"
                    ...
                    <x-core::card class="h-100">
                        ...
                        <x-core::card.body class="d-flex flex-column justify-content-between">
                            ...
                            <div class="row g-1 g-lg-0">
                                @if (!config('libs.plugin-management.general.hide_plugin_author', false) && $plugin->author)
                                    <div class="col-12 col-lg">
                                        {{ trans('libs/plugin-management::plugin.author') }}:
                                        @if ($plugin->url)
                                ...
                                @if ($plugin->version)
                                    <div class="col-12 col-lg-auto">
                                        {{ trans('libs/plugin-management::plugin.version') }}:
                        ...
                        <x-core::card.footer>
                            <div class="btn-list">
                                @if (auth()->user()->hasPermission('plugins.edit'))
                                    ...
                                    >
                                        @if ($plugin->status)
                                            {{ trans('libs/plugin-management::plugin.deactivate') }}
                                        @else
                                            {{ trans('libs/plugin-management::plugin.activate') }}
                                        @endif
                                    </x-core::button>
                                @endif
                                ...
                                @if ($isEnabledMarketplaceFeature)
                                    ...
                                    >
                                        {{ trans('libs/plugin-management::plugin.update') }}
                                    </x-core::button>
                                @endif
                                ...
                                @if (auth()->user()->hasPermission('plugins.remove'))
                                    ...
                                    >
                                        {{ trans('libs/plugin-management::plugin.remove') }}
                                    </x-core::button>
                                @endif
                            </div>
                        </x-core::card.footer>
...
@push('footer')
    <x-core::modal.action
        ...
        type="danger"
        :title="trans('libs/plugin-management::plugin.remove_plugin')"
        :description="trans('libs/plugin-management::plugin.remove_plugin_confirm_message')"
        :submit-button-attrs="['id' => 'confirm-remove-plugin-button']"
        :submit-button-label="trans('libs/plugin-management::plugin.remove_plugin_confirm_yes')"
    />

    @if ($isEnabledMarketplaceFeature)
        <x-core::modal
            id="confirm-install-plugin-modal"
            :title="trans('libs/plugin-management::plugin.install_plugin')"
            button-id="confirm-install-plugin-button"
            :button-label="trans('libs/plugin-management::plugin.install')"
        >
            <input
                type="hidden"
                name="plugin_name"
