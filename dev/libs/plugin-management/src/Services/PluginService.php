...
namespace Dev\PluginManagement\Services;
...
class PluginService
{
    ...
    public function activate(string $plugin): array
    {
        ...
        if (empty($content)) {
            return [
                'error' => true,
                'message' => trans('libs/plugin-management::plugin.invalid_json'),
            ];
        }
        ...
        if ($minimumCoreVersion && (version_compare($coreVersion, $minimumCoreVersion, '<'))) {
            return [
                'error' => true,
                'message' => trans('libs/plugin-management::plugin.minimum_core_version_not_met', [
                    'plugin' => $pluginName,
        ...
        if (! Arr::get($content, 'ready', 1)) {
            return [
                'error' => true,
                'message' => trans(
                    'libs/plugin-management::plugin.plugin_is_not_ready',
                    ['name' => $pluginName]
                ),
            ];
        }
        ...
        if (! in_array($plugin, $activatedPlugins)) {
            ...
            if ($missingPlugins = array_diff($requiredPlugins, $activatedPlugins)) {
                return [
                    'error' => true,
                    'message' => trans(
                        'libs/plugin-management::plugin.missing_required_plugins',
            ...
            return [
                'error' => false,
                'message' => trans('libs/plugin-management::plugin.activate_success'),
            ];
        }

        return [
            'error' => true,
            'message' => trans('libs/plugin-management::plugin.activated_already'),
        ];
    }
    ...
    protected function validate(string $plugin): array
    {
        ...
        if (! $this->files->isDirectory($location)) {
            return [
                'error' => true,
                'message' => trans('libs/plugin-management::plugin.plugin_not_exist'),
            ];
        }
        ...
        if (! $this->getPluginInfo($plugin)) {
            return [
                'error' => true,
                'message' => trans('libs/plugin-management::plugin.missing_json_file'),
            ];
        }
        ...
        if ($this->isInBlacklist($plugin)) {
            return [
                'error' => true,
                'message' => trans('libs/plugin-management::plugin.plugin_invalid'),
            ];
        }
    ...
    public function publishAssets(string $plugin): array
    {
        ...
        if (! $this->files->isWritable($pluginPath)) {
            return [
                'error' => true,
                'message' => trans(
                    'libs/plugin-management::plugin.folder_is_not_writeable',
                    ['name' => $pluginPath]
                ),
            ];
        }
        ...
        return [
            'error' => false,
            'message' => trans('libs/plugin-management::plugin.published_assets_success', ['name' => $plugin]),
        ];
    }
    ...
    public function remove(string $plugin): array
    {
        ...
        return [
            'error' => false,
            'message' => trans('libs/plugin-management::plugin.plugin_removed'),
        ];
    }
    ...
    public function deactivate(string $plugin): array
    {
        ...
        if (! empty($requiredBy)) {
            return [
                'error' => true,
                'message' => trans(
                    'libs/plugin-management::plugin.required_by_other_plugins',
        ...
        if (in_array($plugin, $activatedPlugins)) {
            ...
            return [
                'error' => false,
                'message' => trans('libs/plugin-management::plugin.deactivated_success'),
            ];
        }

        return [
            'error' => true,
            'message' => trans('libs/plugin-management::plugin.deactivated_already'),
        ];
    }
    ...
    protected function isInBlacklist(string $plugin): bool
    {
        $blacklist = [
            'activator',
            'laravel-activator',
            'laravel-activator-main',
            'shaqi/laravel-activator',
        ];
