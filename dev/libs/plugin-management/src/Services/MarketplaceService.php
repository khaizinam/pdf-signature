...
class MarketplaceService
{
    ...
    {
        if (! config('libs.plugin-management.general.enable_marketplace_feature')) {
            abort(404);
        }
        ...
        try {
            ...
            if ($response->status() !== 200) {
                ...
                throw new Exception(Arr::get($body, 'message') ?: trans('libs/plugin-management::marketplace.could_not_connect'));
            }
            ...
            throw new Exception(trans('libs/plugin-management::marketplace.could_not_connect'));
        }
    }
    ...
    protected function extractFile(string $id, string $name): string
    {
        ...
        if (! $zipper->extract($destination, $pathTo)) {
            throw new Exception(trans('libs/plugin-management::marketplace.unzip_failed'));
        }
