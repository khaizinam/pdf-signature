...
class SlugHelper
{
    ...
    public function registerModule(string|array $model, ?string $name = null): self
    {
        ...
        config(['libs.slug.general.supported' => $supported]);
    ...
    {
        return config('libs.slug.general.supported', []);
    }

    public function setPrefix(string $model, ?string $prefix, bool $canEmptyPrefix = false): self
    {
        $prefixes = config('libs.slug.general.prefixes', []);
        $prefixes[$model] = $prefix;

        config(['libs.slug.general.prefixes' => $prefixes]);
    ...
    {
        $columns = config('libs.slug.general.slug_generated_columns', []);
        $columns[$model] = $column;

        config(['libs.slug.general.slug_generated_columns' => $columns]);
    ...
    public function disablePreview(array|string $model): self
    {
        ...
        config([
            'libs.slug.general.disable_preview' => array_merge(
                config('libs.slug.general.disable_preview', []),
                $model
            ),
        ]);
    ...
    {
        return ! in_array($model, config('libs.slug.general.disable_preview', []));
    }
    ...
    public function getPrefix(string $model, string $default = '', bool $translate = true): ?string
    {
        ...
        if ($prefix === null) {
            $prefix = Arr::get(config('libs.slug.general.prefixes', []), $model);
        }
    ...
    public function getHelperText(string $prefix, ?string $postfix = '', ?string $separation = ''): string
    {
        ...
        return trans('libs/slug::slug.settings.helper_text', [
    ...
    public function getColumnNameToGenerateSlug(array|string|object|null $model): ?string
    {
        ...
        $config = Arr::get(config('libs.slug.general.slug_generated_columns', []), $model);
    ...
    {
        $endingURL = setting($this->getSettingKey('public_single_ending_url'), config('libs.theme.general.public_single_ending_url'));
    ...
    public function getAllPrefixes(): array
    {
        ...
        foreach ($this->supportedModels() as $class => $model) {
            ...
            $prefixes[] =  Arr::get(config('libs.slug.general.prefixes', []), $class);
        }
