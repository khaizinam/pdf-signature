...
class ThemeService
{
    ...
    public function activate(string $theme): array
    {
        ...
        if (setting('theme') && $theme == Theme::getThemeName()) {
            return [
                'error' => true,
                'message' => trans('libs/theme::theme.theme_activated_already', ['name' => $theme]),
            ];
        }
        ...
        try {
            ...
            if ($inheritTheme && ! Theme::exists($inheritTheme)) {
                return [
                    'error' => true,
                    'message' => trans('libs/theme::theme.theme_inherit_not_found', ['name' => $inheritTheme]),
                ];
            }
        ...
        return [
            'error' => false,
            'message' => trans('libs/theme::theme.active_success', ['name' => $theme]),
        ];
    }
    ...
    protected function validate(string $theme): array
    {
        ...
        if (! $this->files->isDirectory($location)) {
            return [
                'error' => true,
                'message' => trans('libs/theme::theme.theme_is_not_existed'),
            ];
        }
        ...
        if (! $this->files->exists($location . '/theme.json')) {
            return [
                'error' => true,
                'message' => trans('libs/theme::theme.missing_json_file'),
            ];
        }

        return [
            'error' => false,
            'message' => trans('libs/theme::theme.theme_invalid'),
        ];
    }
    ...
    public function publishAssets(?string $theme = null): array
    {
        ...
        return [
            'error' => false,
            'message' => trans('libs/theme::theme.published_assets_success', ['themes' => implode(', ', $themes)]),
        ];
    }
    ...
    public function remove(string $theme): array
    {
        ...
        if (Theme::getThemeName() === $theme) {
            return [
                'error' => true,
                'message' => trans('libs/theme::theme.cannot_remove_theme', ['name' => $theme]),
            ];
        }
        ...
        if (Theme::getInheritTheme() === $theme) {
            return [
                'error' => true,
                'message' => trans('libs/theme::theme.cannot_remove_inherit_theme', ['name' => $theme]),
            ];
        }
        ...
        return [
            'error' => false,
            'message' => trans('libs/theme::theme.theme_deleted', ['name' => $theme]),
        ];
    }
    ...
    public function removeAssets(string $theme): array
    {
        ...
        $this->files->deleteDirectory(public_path('ui/' . $theme));

        return [
            'error' => false,
            'message' => trans('libs/theme::theme.removed_assets', ['name' => $theme]),
        ];
    }
