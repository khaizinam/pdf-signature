...
use Dev\Theme\Exceptions\UnknownPartialFileException;
use Dev\Theme\Exceptions\UnknownThemeException;
use Dev\Theme\Supports\SocialLink;
use Dev\Theme\Supports\ThemeSupport;
use Dev\Theme\Typography\Typography;
...
class Theme implements ThemeContract
{
    ...
    {
        if (! $this->themeConfig) {
            $this->themeConfig = $this->config->get('libs.theme.general', []);
        }
    ...
    public function header(): string
    {
        ...
        return $this->view->make('libs/theme::partials.header')->render();
    }

    public function footer(): string
    {
        return $this->view->make('libs/theme::partials.footer')->render();
    }
    ...
    public function getLogoImage(array $attributes = [], string $logoKey = 'logo', int $maxHeight = 0, ?string $logoUrl = null): ?HtmlString
    {
        ...
        return apply_filters('theme_logo_image', AppMedia::image($logo, $this->getSiteTitle(), attributes: $attributes));
    }
