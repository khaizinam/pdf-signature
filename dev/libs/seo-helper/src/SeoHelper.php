...
class SeoHelper implements SeoHelperContract
{
    ...
    {
        if (
            in_array(get_class($object), config('libs.seo-helper.general.supported', [])) &&
            ...
            try {
                ...
                if ($request->hasFile('seo_meta_image_input')) {
                    ...
                    $result = AppMedia::handleUpload($request->file('seo_meta_image_input'), 0, $uploadFolder);
    ...
    {
        try {
            if (in_array(get_class($object), config('libs.seo-helper.general.supported', []))) {
                MetaBox::deleteMetaData($object, 'seo_meta');
            }
    ...
    {
        return config('libs.seo-helper.general.supported', []);
    }
    ...
    public function registerModule(array|string $model): static
    {
        ...
        config([
            'libs.seo-helper.general.supported' => array_merge($this->supportedModules(), $model),
        ]);
    ...
    public function removeModule(array|string $model): static
    {
        ...
        config()->set('libs.seo-helper.general.supported', $supported);
