...
namespace Dev\DataSynchronize\Importer;

use Dev\Base\Facades\Assets;
use Dev\Base\Facades\BaseHelper;
use Dev\DataSynchronize\Concerns\Importer\HasImportResults;
use Dev\DataSynchronize\Contracts\Importer\WithMapping;
use Dev\DataSynchronize\DataTransferObjects\ChunkImportResponse;
use Dev\DataSynchronize\DataTransferObjects\ChunkValidateResponse;
use Dev\DataSynchronize\Exporter\ExampleExporter;
...
abstract class Importer
{
    ...
    public function getAcceptedFiles(): array
    {
        return apply_filters(
            ...
            config('libs.data-synchronize.data-synchronize.mime_types')
        );
    }
    ...
    public function getFileExtensions(): array
    {
        return apply_filters(
            ...
            config('libs.data-synchronize.data-synchronize.extensions')
        );
    }
    ...
    {
        return trans(
            'libs/data-synchronize::data-synchronize.import.heading',
            ['label' => $this->getLabel()]
        );
    }
    ...
    {
        Assets::addStylesDirectly('vendor/core/libs/data-synchronize/css/data-synchronize.css')
            ->addScriptsDirectly('vendor/core/libs/data-synchronize/js/data-synchronize.js')
            ->addScripts('dropzone')
            ->addStyles('dropzone');

        $view = 'libs/data-synchronize::import';

        if ($this->renderWithoutLayout) {
            $view = 'libs/data-synchronize::partials.importer';
        }
    ...
    public function validate(string $fileName, int $offset = 0, int $limit = 100): ChunkValidateResponse
    {
        ...
        if ($count === 0) {
            ...
            $storageFolder = config('libs.data-synchronize.data-synchronize.storage.path');
    ...
    {
        return trans('libs/data-synchronize::data-synchronize.import.importing_message', [
    ...
    {
        return trans('libs/data-synchronize::data-synchronize.import.done_message', [
    ...
    {
        $filePath = sprintf('%s/%s', config('libs.data-synchronize.data-synchronize.storage.path'), $fileName);
    ...
    {
        return Storage::disk(config('libs.data-synchronize.data-synchronize.storage.disk'));
    }
    ...
    protected function resolveMediaImage(string $url, ?string $directory = null): string
    {
        ...
        $result = AppMedia::uploadFromUrl($url, 0, $directory);
