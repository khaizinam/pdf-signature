            ...
            <x-core::card.body>
                @if($exporter->hasDataToExport())
                        ...
                        @if($exporter->allColumnsIsDisabled())
                            <x-core::form.label>
                                {!! BaseHelper::clean(trans(
                                    'libs/data-synchronize::data-synchronize.export.form.all_columns_disabled',
                                    ['columns' => Arr::join(array_map(fn ($column) => "<strong>{$column->getLabel()}</strong>", $exporter->getColumns()), ', ')]
                                )) !!}
                            </x-core::form.label>
                        @else
                            <x-core::form.label>
                                {{ trans('libs/data-synchronize::data-synchronize.export.form.columns') }}
                    ...
                    <x-core::form.radio-list
                        :label="trans('libs/data-synchronize::data-synchronize.export.form.format')"
                        name="format"
                        :options="[
                        'csv' => 'CSV',
                        'xlsx' => 'Excel',
                ...
                @if($exporter->hasDataToExport())
                    <x-core::card.footer>
                        <x-core::button type="submit" color="primary">
                            {{ trans('libs/data-synchronize::data-synchronize.export.form.export_button') }}
                        </x-core::button>
                    </x-core::card.footer>
                @endif
        </x-core::card>
