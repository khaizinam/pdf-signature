<x-core::card class="mt-3">
    <x-core::card.header class="flex-wrap gap-3">
        <x-core::card.title>
            {{ trans('libs/data-synchronize::data-synchronize.import.example.title') }}
        </x-core::card.title>
        ...
        <x-core::card.actions class="d-flex flex-wrap gap-1 ps-0">
            <x-core::form method="post" :url="$importer->getDownloadExampleUrl()">
                <x-core::button type="submit" icon="ti ti-file-type-csv" name="format" value="csv">
                    {{ trans('libs/data-synchronize::data-synchronize.import.example.download', ['type' => 'CSV']) }}
                </x-core::button>
                <x-core::button type="submit" icon="ti ti-file-spreadsheet" name="format" value="xlsx">
                    {{ trans('libs/data-synchronize::data-synchronize.import.example.download', ['type' => 'Excel']) }}
                </x-core::button>
            </x-core::form>
        </x-core::card.actions>
    </x-core::card.header>
