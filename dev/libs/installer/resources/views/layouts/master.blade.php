...
<head>
    ...
    >
    <title>@yield('pageTitle', trans('libs/installer::installer.title'))</title>

    <meta
        name="description"
        content="Installation Laravel CMS v{{ get_cms_version() }}"
    >
    ...
    <link
        href="{{ asset('vendor/core/libs/installer/css/style.css') }}?v={{ get_cms_version() }}"
        rel="stylesheet"
    />
                ...
                <div class="row mb-6">
                    <div class="col">
                        <h3 class="h1 page-title justify-content-center text-white">
                            {{ trans('libs/installer::installer.title') }}
                        </h3>
                    </div>
                </div>
                ...
                <div class="row installer-wrapper">
                    ...
                    <div class="col-md-9 p-0">
                        <x-core::card class="h-100">
                            ...
                            <x-core::card.body>
                                @include('libs/installer::partials.alert')
        ...
        @if (Session::has('success_msg') || Session::has('error_msg') || (isset($errors) && $errors->any()) || isset($error_msg))
            <script type="text/javascript">
                $(function() {
                    ...
                    Apps.showSuccess('{{ session('success_msg') }}');
                    @endif
                    @if (Session::has('error_msg'))
                    Apps.showError('{{ session('error_msg') }}');
                    @endif
                    @if (isset($error_msg))
                    Apps.showError('{{ $error_msg }}');
                    @endif
                    @if (isset($errors))
                    @foreach ($errors->all() as $error)
                    Apps.showError('{{ $error }}');
                    @endforeach
                    @endif
                })
            </script>
