...
namespace Dev\Installer\InstallerStep;
    ...
    public static function getItems(): array
    {
        self::$steps = [
            'welcome' => InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.welcome.title'))
                ->route('installers.welcome')
                ->priority(10),
            'requirements' => InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.requirements.title'))
                ->route('installers.requirements.index')
                ->priority(20),
            'environment' => InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.environment.wizard.title'))
                ->route('installers.environments.index')
                ->priority(30),
            'create-account' => InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.createAccount.title'))
                ->route('installers.accounts.index')
                ->priority(50),
            'license' => InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.license.title'))
                ->route('installers.licenses.index')
                ->priority(60),
            'final' => InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.final.title'))
        ...
        if (class_exists(Manager::class) && count(Manager::getThemes()) > 1) {
            self::$steps['theme'] = InstallerStepItem::make()
                ->label(fn () => trans('libs/installer::installer.theme.title'))
