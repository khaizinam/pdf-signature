...
>
    ...
    <div class="dd3-content d-flex justify-content-between">
        ...
        <div class="text-end me-5">{{ Arr::last(explode('\\', (string) $row->reference_type)) ?: trans('libs/menu::menu.custom_link') }}</div>
        <a class="show-item-details" href="#">
            <x-core::icon name="ti ti-chevron-down" />
        </a>
    </div>
    <div class="item-details">
        {!! Dev\Menu\Forms\MenuNodeForm::createFromModel($row)->renderForm([], false, true, false) !!}
        <div class="text-end mt-2">
            <a
                class="btn btn-danger btn-remove btn-sm"
                href="#"
            >{{ trans('libs/menu::menu.remove') }}</a>
            <a
                class="btn btn-primary btn-cancel btn-sm"
                href="#"
            >{{ trans('libs/menu::menu.cancel') }}</a>
        </div>
    </div>
